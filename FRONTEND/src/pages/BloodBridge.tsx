...
export default function BloodBridge() {
  ...
  const handleRequestDonor = async (donorId: string) => {
    try {
      ...
      // We'll use a generic match record for direct hospital->donor request
      await (api as any).blood.postRequest({
        hospital_id: hospitalId,
        donor_id: donorId,
        blood_group: searchGroup || "O-", // Use current filter or fallback
        units: 1,
        urgency: "urgent"
      });
                ...
                <div className="space-y-3">
                  {loading ? (
                    ...
                    <div className="p-10 text-center border-2 border-dashed border-border rounded-2xl">
                      <p className="font-body text-muted-foreground">No active urgent requests. Showing O- as placeholder etc.</p>
                    </div>
                  ) : (
                ...
                <div className="rounded-2xl border-2 border-blood/20 bg-card shadow-card overflow-hidden">
                  <BloodBridgeMap
                    donors={donors.map(d => ({
                      ...
                      lat: 19.0760, // Fallback lat/lng for map pins if donor has none
                      lng: 72.8777,
                    }))}
                  />
                </div>
